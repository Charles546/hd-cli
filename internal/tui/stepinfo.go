	...
	case 14:
		return &stepInfo{
			...
			fields: []fieldDescriptor{
				...
				{
					label:       "Vault role ID",
					placeholder: "role-id-from-vault",
					help:        "VAULT_ROLE_ID - Vault role ID for AppRole authentication",
					getValue:    func(c *config.WizardConfig) string { return c.SecureExecVaultRoleID },
					setValue:    func(c *config.WizardConfig, v string) { c.SecureExecVaultRoleID = v },
					condition:   func(c *config.WizardConfig) bool { return c.GithubCreateRepo && !c.UseLocalCopy && c.SecureExecDriver == "hd-driver-vault" },
				},
				{
					label:       "Vault secret ID",
					placeholder: "secret-id-from-vault",
					help:        "VAULT_SECRET_ID - Vault secret ID for AppRole authentication",
					getValue:    func(c *config.WizardConfig) string { return c.SecureExecVaultSecretID },
					setValue:    func(c *config.WizardConfig, v string) { c.SecureExecVaultSecretID = v },
					condition:   func(c *config.WizardConfig) bool { return c.GithubCreateRepo && !c.UseLocalCopy && c.SecureExecDriver == "hd-driver-vault" },
				},
				{
					label:       "GCP project ID",
					placeholder: "my-gcp-project",
					help:        "Google Cloud project ID for the gcloud-secret driver",
					getValue:    func(c *config.WizardConfig) string { return c.SecureExecGcloudProjectID },
					setValue:    func(c *config.WizardConfig, v string) { c.SecureExecGcloudProjectID = v },
					condition:   func(c *config.WizardConfig) bool { return c.GithubCreateRepo && !c.UseLocalCopy && c.SecureExecDriver == "gcloud-secret" },
				},
			},
		}
	case 15:
