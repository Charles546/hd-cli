...
func StepHelp(step int) string {
	helps := map[int]string{
		...
		2:  "Project name and config directory for your Honeydipper setup.",
		3:  "Docker: easiest setup. Source: build from source. K8s: use Kubernetes.",
		4:  "Optionally create a new GitHub repository for the generated config.",
		5:  "Secure execution driver for fetching secrets at runtime.",
		6:  "Configure clone authentication for the config repository.",
		7:  "Vault is recommended for production. Dev mode stores secrets in env vars.",
		8:  "Local Redis runs in the same container. External uses a separate Redis server.",
		9:  "GitHub integration enables push event triggers and workflow dispatch.",
		10: "Slack integration enables bot notifications and slash commands.",
		11: "AI agent provides natural language workflow triggers and completions.",
		12: "Docker deployment settings. Only applies if Docker mode is selected.",
		13: "Kubernetes deployment settings. Only applies if K8s mode is selected.",
		14: "Source build settings. Only applies if source mode is selected.",
...
func ValidateStepComplete(m *WizardModel) error {
	...
	case 5:
		if cfg.GithubCreateRepo && !cfg.UseLocalCopy {
			driver := strings.TrimSpace(cfg.SecureExecDriver)
			switch driver {
			case "none":
			case "hd-driver-vault":
				if strings.TrimSpace(cfg.SecureExecVaultAddr) == "" {
